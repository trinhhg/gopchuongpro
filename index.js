...
// --- WORKER FOR ANTI-THROTTLING (FIX DELAY) ---
let keepAliveWorker = null;

function initWorker() {
    // Tạo một Worker ảo từ Blob string để tránh phải tạo file .js riêng
    const workerScript = `
        self.onmessage = function(e) {
            if (e.data === 'start') {
                // Gửi tin nhắn 'tick' về main thread mỗi 200ms
                // Việc này ép trình duyệt phải đánh thức main thread để xử lý
    ...
    keepAliveWorker.onmessage = function(e) {
        if (e.data === 'tick') {
            // Mỗi khi worker tick, ta kiểm tra Queue. 
            // Điều này đảm bảo Queue chạy ngay cả khi tab bị ẩn.
...
async function init() {
    await initDB();
    initWorker(); // Khởi động worker chống ngủ
...
async function processQueue() {
    ...
    try {
        ...
        // Gọi lại ngay nếu còn item (Worker cũng sẽ gọi, nhưng gọi ở đây cho nhanh)
...
async function performMerge(task) {
    ...
    if (targetFile) {
        ...
        if (existingIndex !== -1) {
            ...
            addToLog(`Đã cập nhật: ${inputTitle}`, 'warn');
        } else {
            targetFile.segments.push(segment);
            addToLog(`Đã gộp: ${inputTitle}`, 'success');
        }
...
// --- CHECKLIST LOGIC & DUPLICATE DETECTION (FIXED) ---
function importChecklist() {
    try {
        ...
        // 1. Clear lịch sử cũ để làm báo cáo mới
        historyLogs = [];
        clearStore('history');
        ...
        newItems.forEach(item => {
            ...
            if (countMap[item.num] === 2) {
                // Chỉ push vào mảng trùng khi đếm được lần thứ 2 (tránh duplicate trong mảng trùng)
        ...
        if (duplicates.length > 0) {
            // Sắp xếp số chương trùng tăng dần
            duplicates.sort((a,b) => a - b);
            const msg = `⚠️ PHÁT HIỆN TRÙNG: Chương ${duplicates.join(', ')}`;
            addToLog(msg, 'error');
            toast('Phát hiện chương trùng! Kiểm tra tab Thông báo.', 'danger');
        } else {
            addToLog(`✅ Kiểm tra danh sách: ${newItems.length} chương. Không trùng.`, 'success');
            toast(`Đã nhập ${newItems.length} chương. Không trùng.`);
        }

        // 4. Lưu checklist
        let currentList = checklists[currentFolderId] || [];
        // Merge thông minh: chỉ thêm nếu chưa có trong list hiện tại (hoặc replace tuỳ logic, ở đây giữ logic thêm)
        // Nhưng ở đây ta cứ lấy list mới nhất làm chuẩn
        ...
        // 5. Render
        renderChecklist(); 
        renderHistory(); // Render ngay lịch sử để thấy báo cáo
        
        // Nếu có trùng, tự động chuyển sang tab lịch sử để người dùng thấy
        if (duplicates.length > 0) {
            switchView('history');
        } else {
...
function renderHistory() {
    ...
    if(filtered.length === 0) {
         // Không hiển thị gì hoặc hiển thị empty state
    } else {
...
// ... (Các hàm getAll, saveDB, delDB, clearStore, renderFolders, createFolder, deleteCurrentFolder, getFilteredFiles, renderFiles, generateDocxFromSegments giữ nguyên như cũ) ...
...
// ... (Các hàm window.openPreview, closePreview, navChapter, download, delete, toast, switchView giữ nguyên) ...
...
function toast(m, type='info') { 
    ...
    els.toast.className = `toast show ${type}`; // Support danger toast
