...
// --- HISTORY SYSTEM (UPDATED: FOLDER FILTER) ---
function addLog(type, msg) {
    const now = new Date();
    const time = now.toLocaleTimeString('vi-VN', {hour:'2-digit', minute:'2-digit', second:'2-digit'});
    // THÊM FOLDER ID VÀO LOG
    const item = { id: Date.now(), time, type, msg, timestamp: now.getTime(), folderId: currentFolderId };
    logs.unshift(item);
...
function renderHistory() {
    // LỌC THEO FOLDER ID
    let filtered = logs.filter(l => (!l.folderId || l.folderId === currentFolderId));
    
    if (currentFilter === 'scan') filtered = filtered.filter(l => l.type.startsWith('scan'));
    else if (currentFilter === 'merge') filtered = filtered.filter(l => l.type === 'merge');
...
function generateDocxFromSegments(mainHeader, segments) { 
    ...
    children.push(new Paragraph({
        children: [new TextRun({text: mainHeader, font: "Calibri", size: 32, bold: false, color: "000000"})], 
...
window.openPreview = (id) => { 
    ...
    els.previewDocHeader.style.fontWeight = 'normal';
    els.previewDocHeader.style.marginBottom = '1em'; 

    let content = "<div style='height: 1.2em'></div>";
    if(f.segments) {
...
function renderFiles() { 
    ...
    list.forEach(f => { 
        ...
        if (displayName.includes('-')) {
             ...
             if (parts[0].trim().startsWith('Chương')) displayName = parts[0].trim();
        }
        const card = document.createElement('div'); 
        card.className = `file-card ${f.selected ? 'selected' : ''}`; 
        card.onclick = (e) => { if(e.target.closest('.action-pill')) return; f.selected = !f.selected; renderFiles(); }; 
        card.innerHTML = `
...
function clearHistory() { if(confirm("Xóa nhật ký?")) { logs = logs.filter(l => l.folderId !== currentFolderId); clearStore('history'); logs.forEach(l => saveDB('history', l)); renderHistory(); toast("Đã dọn dẹp"); } }
